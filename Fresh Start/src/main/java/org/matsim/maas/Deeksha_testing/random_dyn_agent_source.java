...
// public class random_dyn_agent_source {
...
public class random_dyn_agent_source implements AgentSource {
	private static final int AGENT_COUNT = 100;

	private final QSim qSim;

	@Inject
	public random_dyn_agent_source(QSim qSim) {
		this.qSim = qSim;
	}
	...
	public void insertAgentsIntoMobsim() {
		...
		for (int i = 0; i < AGENT_COUNT; i++) {
			random_dyn_agent_logic agentLogic = new random_dyn_agent_logic(network);

			Id<Person> id = Id.createPersonId(i);
			Id<Link> startLinkId = random_dyn_agent_logic.chooseRandomElement(network.getLinks().keySet());
